...
begin
  ...
  result := jsonb_build_object(
    ...
    'currentTurn', 0, 'scores', '[0,0]'::jsonb,
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare room public.game_rooms; slot integer; next_state jsonb; action_type text := p_action->>'type'; correct boolean; score integer;
begin
  select * into room from public.game_rooms where room_code = p_room_code for update;
  if not found then raise exception 'ROOM_NOT_FOUND'; end if;
  if room.expires_at <= now() then raise exception 'ROOM_EXPIRED'; end if;
  ...
  if action_type = 'ready' then
    ...
    next_state := jsonb_set(next_state, '{question}', p_action->'question');
    ...
    correct := coalesce(room.state#>'{question,validPlayerIds}', jsonb_build_array((room.state#>>'{question,correctPlayerId}')::bigint)) @> jsonb_build_array((p_action->>'selectedPlayerId')::bigint);
  ...
  next_state := jsonb_set(next_state, '{stateVersion}', to_jsonb(room.state_version + 1));
  update public.game_rooms set state = next_state, state_version = state_version + 1, updated_at = now() where room_code = p_room_code;
  return next_state;
end $$;
